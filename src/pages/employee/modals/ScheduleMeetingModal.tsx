...
const ScheduleMeetingModal = ({ open, onClose, onAdd }: ScheduleMeetingModalProps) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { resetForm }) => {
            try {
                const filter: { role?: string; department?: string } =
                    values.role
                        ? { role: values.role }
                        : { department: values.department };

                const payload: ScheduleMeeting = {
                    title: values.title,
            ...
            <DialogContent className="rounded-xl max-w-md max-h-[80vh] overflow-y-auto scrollbar-hidden">
                ...
                <form onSubmit={formik.handleSubmit} className="space-y-4">
                    ...
                    <Button type="submit" className="w-full bg-blue-600 text-white" disabled={formik.isSubmitting}>
                        {formik.isSubmitting ? "Scheduling..." : "Schedule Meeting"}
                    </Button>
                </form>
            </DialogContent>
        </Dialog>
